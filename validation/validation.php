...
// Define checkWord() here:
  function checkWord($user_input, $first_letter) {
    if ($_SERVER["REQUEST_METHOD"] === "POST" && strtolower($user_input[0]) !== $first_letter) {
      return "* This word must start with the letter ${first_letter}!";
    } else {
      return "";
    }
  };
...
<form method="post" action="">
    ...
    <br>
    <input type="text" id="b-word" name="b-word" value="<?= $_POST["b_word"];?>">
    <br>      
      <p class="error" id="b-error"><?= checkWord($_POST["b-word"], "b") ?></p> 
    <br>
    Enter a word that starts with the letter "c":
    <br>
    <input type="text" id="c-word" name="c-word" value="<?= $_POST["c_word"];?>">
    <br>      
